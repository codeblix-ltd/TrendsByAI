...
import { GraduationCapIcon, EyeIcon, ThumbsUpIcon, ClockIcon } from '../icons';
...
const VideoItem: React.FC<{ video: Video; onGenerateSEO: (video: Video) => void }> = ({ video, onGenerateSEO }) => {
  ...
  return (
    <div className="flex items-center gap-4 py-3 border-b border-white/10 last:border-b-0">
      <div className="relative w-24 h-24 flex-shrink-0">
        <img 
          src={video.thumbnail_url || 'https://via.placeholder.com/96x96?text=No+Image'} 
          alt={video.title} 
          className="w-full h-full rounded-xl object-cover"
        />
        <div className="absolute top-1.5 left-1.5 flex flex-col items-start gap-1">
          {recentBadge && (
            <span className={`text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg ${recentBadge.color}`}>
              {recentBadge.label}
            </span>
          )}
          {viralBadge && (
            <span className={`text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg flex items-center gap-0.5 ${viralBadge.color}`}>
      ...
      <div className="flex-grow min-w-0">
        <a 
          ...
          className="font-semibold text-white hover:text-blue-400 transition-colors line-clamp-2 block"
        >
          {video.title}
        </a>
      ...
      <div className="flex flex-col gap-2">
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer" 
          className="flex items-center gap-1 text-sm px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-colors"
        >
        ...
        <button
          ...
          className="flex items-center gap-1 text-sm px-3 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/20 rounded-full transition-colors text-blue-300"
        >
...
const VideoContentCard: React.FC<VideoContentCardProps> = ({ videos, onGenerateSEO }) => {
      ...
      <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2">
        {sortedVideos.length > 0 ? (
          sortedVideos.slice(0, 20).map(video => (
            <VideoItem key={video.id} video={video} onGenerateSEO={onGenerateSEO} />
          ))
        ) : (
